...
public class Fiche {
	...
	public static final String XML_REF_DEFINITIONSUTILISEES = "definitionsUtilisees";
...
4 : Fiche Publiée - 1, 2, 3 : En cours de Rédaction - 5 : Fiche Proposée */ 
	...
	/** Liste des défintions du glossaire utilisées dans cette fiche */ 
	// work in progress, find association 
	...
	/** Liste des défintions du glossaire utilisées dans cette fiche 
	  * Attention, returned list is readonly
      */
	public List<DefinitionGlossaire> getDefinitionsUtilisees(){
		List<DefinitionGlossaire> result = new ArrayList<DefinitionGlossaire>();
		...
		for (Fiches_DefinitionsGlossaire aFiches_DefinitionsGlossaire : fiches_DefinitionsGlossaire) {
			...
			result.add(aFiches_DefinitionsGlossaire.getDefinitionGlossaire());
		}
		return result;
	}
	public void addDefinitionGlossaire(DefinitionGlossaire definitionGlossaire){
		try {
			_contextDB.fiches_DefinitionsGlossaireDao.create(new Fiches_DefinitionsGlossaire( this, definitionGlossaire));
		} catch (SQLException e) {
	...
	public String toXML(String indent, DorisDBHelper contextDB){
		...
		for(DefinitionGlossaire ref : this.getDefinitionsUtilisees()){
    		sb.append("\n"+indent+"\t<"+XML_REF_DEFINITIONSUTILISEES+" id=\"");
    		sb.append(ref._id);
        	sb.append("\"/>");
